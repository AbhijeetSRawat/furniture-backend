...
// Main receipt generation function using Puppeteer
export const downloadReceipt = async (req, res) => {
  let browser = null;
...
// Lightweight fallback using a different approach
export const downloadReceiptFallback = async (req, res) => {
  try {
    console.log('Using fallback method for receipt generation');
    ...
    // For environments where Puppeteer might not work,
    // you can use jsPDF or return HTML directly
    res.setHeader('Content-Type', 'text/html');
    res.setHeader('Content-Disposition', `attachment; filename="receipt-${order.orderId}.html"`);
...
// Alternative using @react-pdf/renderer for server environments
export const downloadReceiptReactPDF = async (req, res) => {
  // This would require: npm install @react-pdf/renderer
  // Implementation using React PDF for better server compatibility
  try {
    const orderDoc = await Order.findById(req.params.id)
      .populate("user", "name phone")
      .populate("shippingAddress");
    ...
    // You would implement React PDF components here
    // This is just a placeholder showing the structure
    
    console.log('React PDF method would be implemented here');
    res.status(501).json({ message: "React PDF method not yet implemented" });
